...
- ориентир на 1 digest-запрос через Claude Haiku: около `0.1 цента` при компактном shortlist
...
1. Подписка для founders и PM, которым нужен мониторинг конкурентной среды и рынка
2. Командный тариф для аналитиков, агентств и research-команд
3. Вертикальный продукт для ниш вроде crypto / OSINT / региональных медиа

### Первый канал привлечения

Если запускать это как коммерческий продукт, первый канал привлечения — сам Telegram:

- короткое demo-видео
- простой landing page
- ручной outreach в релевантные Telegram-сообщества

### Что показать в demo-видео

Для сдачи достаточно 2-минутного ролика со сценарием:

1. показать запущенный бот
2. отправить `/start`
3. добавить канал через `@channelname`
4. добавить канал через `https://t.me/...`
5. показать `/list`
6. запустить `/digest`
7. показать 5 тем, краткие описания и ссылки
8. удалить канал через `/remove`
